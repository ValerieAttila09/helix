...
export function DocsSidebar() {
  ...
  return (
    <aside className="hidden md:block w-64 px-8 shrink-0 sm:mt-24 overflow-y-auto py-10 border-r border-neutral-800">
      <ul className="space-y-2">
        {Object.entries(DOCS_SIDEBAR_LINKS).map(([title, links]) => (
          <li key={title}>
            <h4 className="mb-2 font-semibold text-neutral-400 sticky top-0 bg-black py-1.5">{title}</h4>
